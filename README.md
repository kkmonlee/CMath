...
Use it to evaluate mathematical expressions at runtime and bind variables without generating code by hand. The
interpreter works anywhere a C compiler runs. The JIT is optional and currently targets Apple Silicon (aarch64) first.
...
- Working on other platforms actively.
...
- In the native reference, it is better to prefer `__builtin_fma`, `__builtin_fabs` (or enable `-ffp-contract=fast`) to
  avoid libm calls that inhibit vectorization.
...
- Math functions:
  abs (fabs), acos, asin, atan, atan2, ceil, cos, cosh, exp, floor, ln (log), log (log10), pow, sin, sinh, sqrt, tan,
  tanh
- Constants: `pi`, `e`
